...
export async function GET(req: Request, context: RouteContext) {
  try {
    ...
    const data = await apiFetchWithAuth(`/mitras/${mitraId}/class`, {
      method: 'GET',
    });
...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    const apiError = error as { status?: number; code?: string; message?: string; errors?: ApiErrorIssue[] };
    return toRouteError(apiError.message ?? 'Failed to create mitra class', {
      status: apiError.status ?? 500,
      code: apiError.code ?? 'CREATE_MITRA_CLASS_FAILED',
      message: apiError.message ?? 'Failed to create mitra class',
      errors: apiError.errors ?? [],
    });
  }
}
