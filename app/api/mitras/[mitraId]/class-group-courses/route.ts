...
export async function GET(req: Request, context: RouteContext) {
  try {
    ...
    const data = await apiFetchWithAuth(`/mitras/${mitraId}/class-group-courses`, {
      method: 'GET',
    });
    ...
    return toRouteError(apiError.message ?? 'Failed to fetch mitra class group course', {
      ...
      message: apiError.message ?? 'Failed to fetch mitra class group courses',
...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    const apiError = error as { status?: number; code?: string; message?: string; errors?: ApiErrorIssue[] };
    return toRouteError(apiError.message ?? 'Failed to create mitra class group course', {
      status: apiError.status ?? 500,
      code: apiError.code ?? 'CREATE_MITRA_CLASS_GROUP_COURSE_FAILED',
      message: apiError.message ?? 'Failed to create mitra class group course',
      errors: apiError.errors ?? [],
    });
  }
}
