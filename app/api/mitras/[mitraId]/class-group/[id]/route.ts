...
export async function GET(req: Request, context: RouteContext) {
  try {
    ...
    const data = await apiFetchWithAuth(`/mitras/${mitraId}/class-group/${id}`, {
      method: 'GET',
    });
...
export async function DELETE(req: Request, context: RouteContext) {
  try {
    ...
    const apiError = error as { status?: number; code?: string; message?: string; errors?: ApiErrorIssue[] };
    return toRouteError(apiError.message ?? 'Failed to delete mitra class group', {
      status: apiError.status ?? 500,
      code: apiError.code ?? 'DELETE_MITRA_CLASS_GROUP_FAILED',
      message: apiError.message ?? 'Failed to delete mitra class group',
      errors: apiError.errors ?? [],
    });
  }
}
...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    const apiError = error as { status?: number; code?: string; message?: string; errors?: ApiErrorIssue[] };
    return toRouteError(apiError.message ?? 'Failed to update mitra class group', {
      status: apiError.status ?? 500,
      code: apiError.code ?? 'UPDATE_MITRA_CLASS_GROUP_FAILED',
      message: apiError.message ?? 'Failed to update mitra class group',
      errors: apiError.errors ?? [],
    });
  }
}
