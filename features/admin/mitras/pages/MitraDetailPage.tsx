...
import { useMitraDetailController } from '../hooks/MitraDetailController';
import { MitraDeleteDialog } from '../components/MitraDeleteDialog';
...
import { InfoMitraCard } from '../components/InfoMitraCard';
import { infoMitraItems } from '../components/InfoMitraCard';
...
export function MitraDetailPage({ mitraId }: MitraDetailPageProps) {
  ...
  const { mitra, isLoading, isError, error, isDeleteOpen, onDeleteDialogChange } = useMitraDetailController(mitraId);
              ...
              {isLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    {infoMitraItems(mitra).map((item) => (
                      <InfoMitraCard key={item.label} icon={item.icon} label={item.label} value={item.value} />
                    ))}
                  </div>
                </div>
              ) : (
