...
export function AccessControlHeader({ userRole, scope, scopeLocked, includeDeleted, onScopeChange, onIncludeDeletedChange, actionButtons }: AccessControlHeaderProps) {
  return (
    <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between mb-4">
      <div className="space-y-1">
        ...
        <p className="max-w-2xl text-sm font-medium text-slate-500">Control access levels and assign roles to your team.</p>
      </div>

      <div className="flex flex-col items-end gap-3 w-full lg:w-auto">
        {actionButtons && <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto justify-end">{actionButtons}</div>}
          ...
          {userRole === 'ADMIN' ||
            (userRole === 'SUPER_ADMIN' && (
              <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm cursor-pointer hover:bg-slate-50 transition-colors">
                <input type="checkbox" className="rounded border-slate-300 text-slate-900 focus:ring-slate-900" checked={includeDeleted} onChange={(event) => onIncludeDeletedChange(event.target.checked)} />
