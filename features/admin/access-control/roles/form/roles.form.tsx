...
import { buildRoleDefaultValues } from '../helper/roles.helpers';
...
export function RolesForm({ scope, role, onSubmit, isLoading, onOpenChange }: RolesFormProps) {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="flex items-center gap-3 rounded-lg border px-3 py-2 md:mt-7">
          <Controller
            control={form.control}
            name="isSystem"
            render={({ field }) => <input id="role-is-system" type="checkbox" className="size-4 rounded border-input" checked={field.value} disabled={isLoading} onChange={(event) => field.onChange(event.target.checked)} />}
          />
