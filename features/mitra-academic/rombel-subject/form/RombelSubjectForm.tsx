...
export function RombelSubjectForm({ form, queries, isSubmitting, handleSubmit, editingItem, handleCloseForm }: RombelSubjectFormProps) {
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      <AcademicSelectField
        label="Kelas"
        value={form.watch('classGroupId')}
        onChange={(value) => form.setValue('classGroupId', value, { shouldValidate: true })}
        options={queries.rombelOptions}
        isLoading={queries.isLoadingRombels}
        isError={queries.isErrorRombels}
        error={readErrorMessage(form.formState.errors, 'classGroupId')}
      />
      <AcademicSelectField
        label="Mata Pelajaran"
        value={form.watch('courseMitraId')}
        onChange={(value) => form.setValue('courseMitraId', value, { shouldValidate: true })}
        options={queries.CourseOptions}
        isLoading={queries.isLoadingSubjects}
        isError={queries.isErrorSubjects}
        error={readErrorMessage(form.formState.errors, 'courseMitraId')}
      />
      <AcademicSelectField
        label="Guru"
        ...
        error={readErrorMessage(form.formState.errors, 'teacherId')}
      />
      <AcademicSelectField
        label="Tahun Ajaran"
